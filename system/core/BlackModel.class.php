...
class BlackModel extends \ActiveRecord\Model{
	...
	public static function search_count($domain=false){
		if($domain){
			return static::count(array('conditions' => $domain));
		}
		return static::count();
	}
		...
		foreach ($field_names as $name) {
			if(array_key_exists($name, $attributes)){
				...
				if($relation->is_poly()){ // hasmany/hasmanythrough/...
					...
					$label = $class_name::$rec_name;
					$field = $class_name::get_has_many_definition($name);
					...
					$fields[$name] = array(
							...
							'through' => array_key_exists('througt', $field) ? $field['through'] : false,
					);
				}else{ // belongs to
	...
	public static function get_has_many_definition($name){
		foreach (static::$has_many as $field){
			if($field[0] == $name){
				return $field;
			}
		}
