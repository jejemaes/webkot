	...
	switch ($_GET['action']) {
		...
		case "managepicture" :
			if (RoleManager::getInstance ()->hasCapabilitySession ( 'activity-manage-picture' ) || RoleManager::getInstance ()->hasCapabilitySession ( 'activity-add-picture' )) {
				...
				throw new AccessRefusedException("Vous ne pouvez pas manager les photos de l'activitŽ.");
			}
			break;
		/*	
		case "publish" :
			...
			if (RoleManager::getInstance ()->hasCapabilitySession ( 'activity-can-censure' )) {
				...
				if(isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])){	
					try{
						...
						$message = new Message(1);
						$message->addMessage("La demande de censure a ŽtŽ supprimŽe avec succes.");
					}catch(Exception $e){
						$message = new Message(3);
