...
(function() {
    ...
    var debug = false;// ($.deparam($.param.querystring()).debug !== undefined);

    var blackcms = window.blackcms = {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: blackcms.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = blackcms.__DEBUG__.get_dependencies( job.name );
                ...
                var missing = blackcms.__DEBUG__.get_missing_jobs();
                var failed = blackcms.__DEBUG__.get_failed_jobs();
                ...
                if (blackcms.debug && !_.isEmpty(debug_jobs)) log.push('\nDebug:                  ', debug_jobs);

                if (blackcms.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                ...
                try {
                    ...
                    $.when(job.factory.call(null, require)).then(
                        function (data) {
                            ...
                            clearTimeout(time);
                            time = _.defer(blackcms.process_jobs, jobs, services);
                            def.resolve();
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
